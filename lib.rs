...
//   Julia set iteration: z_{n+1} = z_{n}^2 + c
...
pub fn phase(z0: Complex, policy: &SecurityPolicy) -> PhaseResult {
    ...
    for i in 0..policy.max_iter {
        ...
        // Track trajectory shape (simple heuristics)
        if norm > prev_norm {
            monotonic_growth += 1;
        }
        ...
        // Sign changes in angular velocity indicate oscillation
...
pub fn growth(z0: Complex, policy: &SecurityPolicy, t: f64, n: f64) -> GrowthResult {
    ...
    // If you want t₀ explicitly, lift this to: exp(-r*(t - t0)).
...
impl FeatureMapper {
    ...
    pub fn map_packet(&self, packet_size: f64, inter_arrival_ms: f64) -> Complex {
        ...
        let log_iat = if inter_arrival_ms < 0.01 {
            ...
            libm::log10(inter_arrival_ms * 100.0) / 6.0 // log10(1) to log10(1_000_000)
        };
